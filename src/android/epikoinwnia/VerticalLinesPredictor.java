...
public class VerticalLinesPredictor extends Activity2 implements OnTouchListener
{
	...
	private boolean checkingVerticalLines=false;
	...
	public VerticalLinesPredictor(TextView text,TextView timerInfo,TextView score,ImageView view,boolean backsidechecked)
	{
		this.text=text;
		...
		this.view=view;
		if(!backsidechecked)
		this.view.setImageResource(R.drawable.cardverticallines);
	...
	public boolean onTouch(View v, MotionEvent event) {
		...
		if(event.getAction()==MotionEvent.ACTION_UP)
		{
			...
			if(new Line(coordinates,view.getWidth()*(0.1f)).predict() && !checkingVerticalLines)
			{
				Log.d("Shape is Line Waiting second shape",String.valueOf(coordinates.size()));
				text.setText("Draw the next line..");
				copyToVec1();
				checkingVerticalLines=true;
				coordinates.clear();
				return true;
			}
									
			if(checkingVerticalLines)
			{
				Log.d("Checking 2n Shape"," asdasdas");
				Line test = new Line(coordinates);
				test.setDeviation(view.getWidth()*(0.1f));
				if(test.predict())
				{
					copyToVec2();
					if(new VerticalLines(vec1,vec2).predict())
						correct=true;
				}
				vec1.clear();
				vec2.clear();
				coordinates.clear();
				checkingVerticalLines=false;
			}
    ...
    private OnTouchListener getRandomListener()
    {
    	...
    	case 3: return new CirclePredictor(text,timerInfo,score,view,false);
    	default: return new VerticalLinesPredictor(text,timerInfo,score,view,false);
    	}
    }
    ...
    private void backside()
    {
    	...
    	view.setOnTouchListener(new VerticalLinesPredictor(text,timerInfo,score,view,true));
	}
    private Handler Myview = new Handler() {
        ...
        public void handleMessage(Message msg) {
			...
			mytimer.start();
        	view.setImageResource(R.drawable.cardverticallines);
        };
    };
}
