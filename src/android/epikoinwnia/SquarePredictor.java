...
public class SquarePredictor extends Activity2 implements OnTouchListener
{
	...
	public SquarePredictor(TextView text,TextView timerInfo,TextView score,ImageView view,boolean backsidechecked)
	{
		this.text=text;
		...
		this.view=view;
		if(!backsidechecked)
		this.view.setImageResource(R.drawable.cardsquare);
	...
	public boolean onTouch(View v, MotionEvent event) {
		...
		if(event.getAction()==MotionEvent.ACTION_UP)
		{
			...
			square=new Square(coordinates);
			square.setDeviation(view.getWidth()*(1/10f));
			if(square.predict())
			{
				correct=true;
				Log.d("Shape  is square"," end");
			}
			else
			{
				Log.d("coordinates size is",String.valueOf(coordinates.size()/4));
				int end = coordinates.size()/4;
				for(int i=1; i<end; i=i+3)
					{
						Log.d("Checking new parameters",String.valueOf(i));
						rearrange(i);
						square=new Square(temp);
						square.setDeviation(view.getWidth()*(2/10f));
						if(square.predict())
						{
							correct=true;
							i=end;
    ...
    private OnTouchListener getRandomListener()
    {
    	...
    	case 2: return new CirclePredictor(text,timerInfo,score,view,false);
    	case 3: return new VerticalLinesPredictor(text,timerInfo,score,view,false);
    	default: return new SquarePredictor(text,timerInfo,score,view,false);
    	}
    }
    private void backside()
    {
    	...
    	view.setOnTouchListener(new SquarePredictor(text,timerInfo,score,view,true));
	}
    private Handler Myview = new Handler() {
        ...
        public void handleMessage(Message msg) {
			...
			mytimer.start();
        	view.setImageResource(R.drawable.cardsquare);
        };
    };
    
    private void rearrange(int step)
    {
    	temp.clear();
    	for(int i=step; i<coordinates.size(); i++)
